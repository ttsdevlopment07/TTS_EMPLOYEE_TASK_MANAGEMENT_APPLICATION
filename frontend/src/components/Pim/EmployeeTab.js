...
function EmployeeTab() {
  ...
  const validateForm = () => {
    ...
    if (createLogin) {
      // Password fields are required only if creating login
  ...
  const handleSave = async () => {
    ...
    try {
      if (isUpdate) {
        await axios.put(`http://localhost:5000/api/employees/${editId}`, employeeData);
      } else {
  ...
  const retireEmployee = async (emp) => {
    if (!window.confirm('Are you sure you want to retire this employee?')) return;
    try {
      await axios.put(`http://localhost:5000/api/employees/${emp.id}`, {
        ...emp,
                ...
                <tr key={idx}>
                  ...
                  <td>
                    {!emp.dol ? (
                      ...
                      <button className="action-btn" style={{ background: '#e53935' }} onClick={() => removeEmployee(emp.id)}>Remove</button>
                    )}
                  </td>
                </tr>
              ))}
            ...
            <div className="form-details-right">
              ...
              <div className="toggle-row">
                <label className="switch">
                  <input type="checkbox" checked={createLogin} onChange={() => setCreateLogin(v => !v)} />
